...
func checkTrieNode(t *testing.T, n Node, ch rune, value int) {
	...
	if l := n.Label(); !Equal(l, KeyRune(ch)) {
...
func TestTrie(t *testing.T) {
	trie := NewTrie()
	trie.Put(KeySeqString("1"), 111)
	trie.Put(KeySeqString("2"), 222)
	trie.Put(KeySeqString("3"), 333)
	trie.Put(KeySeqString("4"), 444)
	trie.Put(KeySeqString("5"), 555)
...
func TestNotFound(t *testing.T) {
	trie := NewTrie()
	if trie.Get(KeySeqString("not_exist")) != nil {
